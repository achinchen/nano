...
const Year = ({ selectedDate, today }: Props) => {
  ...
  const daysInMonth = useMemo(() => monthDaysInYear(selectedYear), [selectedYear]);
            ...
            <li>
              <ol className="grid grid-cols-7 my-3 text-gray-600">
                {DAYS_SHORT.map((DAY, dayIndex) => (
                  // eslint-disable-next-line react/no-array-index-key
                  <li key={`${selectedYear}-${monthIndex}-${DAY}-${dayIndex}`} className="w-8 text-center">
                    {DAY}
                  </li>
                ))}
              </ol>
              ...
              <ol className="grid grid-cols-7 grid-rows-6">
                {days.map(({ month, day }) => (
                  <li
                    ...
                    className={`${month === monthIndex + 1 ? 'text-gray-700' : 'text-gray-400'}`}
                  >
                    <button
