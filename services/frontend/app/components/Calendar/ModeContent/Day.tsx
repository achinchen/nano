...
export function Day({ selectedDate, today }: Props) {
  ...
  const todayIsSelected =
    todayDay === selectedDay && todayMonth === selectedMonth;
    ...
    <Fragment>
      <ol className="sticky top-0 flex justify-around bg-white text-center text-sm">
        {currentWeek.map(({ month, day }, index) => (
          <li
            key={`${month}-${day}-${DAYS[index]}`}
            className="w-1/7 flex flex-col items-center py-2"
          >
      ...
      <ol className="text-sm text-gray-600">
        {HOURS.map((HOUR, index) => (
          ...
          <Fragment key={`${HOUR}-${index}`}>
            <li className="border-b border-t-0 border-gray-300 border-solid p-2">
              {HOUR}
            </li>
          </Fragment>
        ))}
