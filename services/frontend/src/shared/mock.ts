import ExampleStudio from '~frontend/assets/example-studio.png';

const LOCATION = { name: '台北店', address: '台北市中正區忠孝西路一段49號' };
const SUPPLIER = '安老師';

export const STUDIO = {
  name: '日安蛋糕工作室',
  avatar: ExampleStudio,
};

export const STUDIO_TIMES = ['09:00', '20:30'];

export const STUDIO_DURATION = 690;

export const TAKE_LEAVES = [
  {
    duration: STUDIO_DURATION,
    startAt: '2024-01-30T09:00',
  },
];

export const SERVICE = {
  IN_PROGRESS: [
    {
      queue: true,
      attendee: 1,
      supplier: SUPPLIER,
      location: LOCATION,
      duration: 45,
      name: '2024 塔羅預約 - 45 分鐘',
      currentAttendee: 0,
      allday: true,
      serviceId: 10,
      startAt: '2024-01-01T10:00',
      endAt: '2024-12-31T00:00',
      description: `
      🔮 洞悉未來之旅 🔮
  【簡介】 
  歡迎蒞臨我們的塔羅預約！在這場神秘的旅程中，您將探索塔羅牌背後的奧秘與智慧。我們將引領您進入塔羅牌的世界。
  事業 🌕 感情 🌖 財富 🌔 心靈成長
  安老師將為您提供精準的解讀和個人化的指引，助您找到生活中的方向和平衡，啟動您的內在探索之旅！
  
  【服務費用】
  45 分鐘：800元
  60 分鐘：1000元 - 請至 60 分課程頁面中預約
  90 分鐘：1200元 - 請至 90 分課程頁面中預約
  付款方式：在預約確認後，我們將提供您專屬的付款帳號。請務必於收到付款帳號後的三天內完成付款程序，以確保您的訂單有效。逾期未完成付款者，將取消訂單。
  
  【預約限制】
  預約限制：每日名額有限，請盡早預約確保您的位置。
  人數限制：由於空間有限及隱私，不建議攜伴超過三人。若有特殊需求，請提前告知。
  為維護空間整潔及環境品質，請勿在教室內進食。
  
  【課程提醒】 
  這是一場個人尋找心靈指引的旅程。請確認您已了解上課地點的詳細地址和相關交通資訊，提前安排好您的交通方式，以確保準時抵達上課地點。`,
    },
    {
      queue: true,
      attendee: 1,
      supplier: SUPPLIER,
      location: LOCATION,
      duration: 60,
      name: '2024 塔羅預約 - 60 分鐘',
      currentAttendee: 0,
      allday: true,
      serviceId: 11,
      startAt: '2024-01-01T10:00',
      endAt: '2024-12-31T00:00',
      description: `
      🔮 洞悉未來之旅 🔮
  【簡介】 
  歡迎蒞臨我們的塔羅預約！在這場神秘的旅程中，您將探索塔羅牌背後的奧秘與智慧。我們將引領您進入塔羅牌的世界。
  事業 🌕 感情 🌖 財富 🌔 心靈成長
  安老師將為您提供精準的解讀和個人化的指引，助您找到生活中的方向和平衡，啟動您的內在探索之旅！
  
  【服務費用】
  45 分鐘：800元 - 請至 45 分課程頁面中預約
  60 分鐘：1000元 
  90 分鐘：1200元 - 請至 90 分課程頁面中預約
  付款方式：在預約確認後，我們將提供您專屬的付款帳號。請務必於收到付款帳號後的三天內完成付款程序，以確保您的訂單有效。逾期未完成付款者，將取消訂單。
  
  【預約限制】
  預約限制：每日名額有限，請盡早預約確保您的位置。
  人數限制：由於空間有限及隱私，不建議攜伴超過三人。若有特殊需求，請提前告知。
  為維護空間整潔及環境品質，請勿在教室內進食。
  
  【課程提醒】 
  這是一場個人尋找心靈指引的旅程。請確認您已了解上課地點的詳細地址和相關交通資訊，提前安排好您的交通方式，以確保準時抵達上課地點。`,
    },
    {
      queue: true,
      attendee: 1,
      supplier: SUPPLIER,
      location: LOCATION,
      duration: 240,
      name: '🔮 塔羅靈性啟發指導 🔮 ',
      currentAttendee: 0,
      allday: false,
      serviceId: 12,
      startAt: '2024-01-01T13:00',
      endAt: '2024-12-31T17:00',
      description: `
      【課程簡介】 
  歡迎您預約塔羅啟發培訓課程！這是一場塔羅深度培訓，專為您打造的個人化心靈探索之旅。在這四小時的課程中，您將與安老師深入探討塔羅牌的奧秘，學會如何解讀牌面的象徵意義，並掌握塔羅的應用技巧，啟發您的內在力量，指引您走向心靈成長之路。
  
  【服務費用】
  8,800元
  付款方式：在預約確認後，我們將提供您專屬的付款帳號。請在收到付款帳號後的三天內完成付款程序，以確保您的訂單有效。逾期未完成付款者，將取消訂單。
  
  【預約限制】
  預約限制：每月名額有限，請盡早預約確保您的位置。
  人數限制：課程設計為一對一培訓，提供專屬的學習體驗。
  
  【課程提醒】 
  這是一場個人化的心靈啟發之旅。請確認您已了解上課地點的詳細地址和相關交通資訊，提前安排好您的交通方式，以確保準時抵達上課地點。 
      `,
    },
  ],
  END: [
    {
      queue: true,
      attendee: 1,
      supplier: SUPPLIER,
      location: LOCATION,
      duration: 45,
      name: '2023 塔羅預約 - 45 分鐘',
      currentAttendee: 1,
      allday: true,
      serviceId: 5,
      startAt: '2023-01-01T10:00',
      endAt: '2023-12-31T00:00',
      description: `
      🔮 洞悉未來之旅 🔮
  【簡介】 
  歡迎蒞臨我們的塔羅預約！在這場神秘的旅程中，您將探索塔羅牌背後的奧秘與智慧。我們將引領您進入塔羅牌的世界。
  事業 🌕 感情 🌖 財富 🌔 心靈成長
  安老師將為您提供精準的解讀和個人化的指引，助您找到生活中的方向和平衡，啟動您的內在探索之旅！
  
  【服務費用】
  45 分鐘：800元
  60 分鐘：1000元 - 請至 60 分課程頁面中預約
  90 分鐘：1200元 - 請至 90 分課程頁面中預約
  付款方式：在預約確認後，我們將提供您專屬的付款帳號。請務必於收到付款帳號後的三天內完成付款程序，以確保您的訂單有效。逾期未完成付款者，將取消訂單。
  
  【預約限制】
  預約限制：每日名額有限，請盡早預約確保您的位置。
  人數限制：由於空間有限及隱私，不建議攜伴超過三人。若有特殊需求，請提前告知。
  為維護空間整潔及環境品質，請勿在教室內進食。
  
  【課程提醒】 
  這是一場個人尋找心靈指引的旅程。請確認您已了解上課地點的詳細地址和相關交通資訊，提前安排好您的交通方式，以確保準時抵達上課地點。`,
    },
    {
      queue: true,
      attendee: 1,
      supplier: SUPPLIER,
      location: LOCATION,
      duration: 60,
      name: '2023 塔羅預約 - 60 分鐘',
      currentAttendee: 0,
      allday: true,
      serviceId: 6,
      startAt: '2023-01-01T10:00',
      endAt: '2023-12-31T00:00',
      description: `
      🔮 洞悉未來之旅 🔮
  【簡介】 
  歡迎蒞臨我們的塔羅預約！在這場神秘的旅程中，您將探索塔羅牌背後的奧秘與智慧。我們將引領您進入塔羅牌的世界。
  事業 🌕 感情 🌖 財富 🌔 心靈成長
  安老師將為您提供精準的解讀和個人化的指引，助您找到生活中的方向和平衡，啟動您的內在探索之旅！
  
  【服務費用】
  45 分鐘：800元 - 請至 45 分課程頁面中預約
  60 分鐘：1000元 
  90 分鐘：1200元 - 請至 90 分課程頁面中預約
  付款方式：在預約確認後，我們將提供您專屬的付款帳號。請務必於收到付款帳號後的三天內完成付款程序，以確保您的訂單有效。逾期未完成付款者，將取消訂單。
  
  【預約限制】
  預約限制：每日名額有限，請盡早預約確保您的位置。
  人數限制：由於空間有限及隱私，不建議攜伴超過三人。若有特殊需求，請提前告知。
  為維護空間整潔及環境品質，請勿在教室內進食。
  
  【課程提醒】 
  這是一場個人尋找心靈指引的旅程。請確認您已了解上課地點的詳細地址和相關交通資訊，提前安排好您的交通方式，以確保準時抵達上課地點。`,
    },
    {
      queue: true,
      attendee: 1,
      supplier: SUPPLIER,
      location: LOCATION,
      duration: 240,
      name: '🔮 塔羅靈性啟發指導 🔮 ',
      currentAttendee: 0,
      allday: false,
      serviceId: 8,
      startAt: '2023-12-01T13:00',
      endAt: '2023-12-31T17:00',
      description: `
      【課程簡介】 
  歡迎您預約塔羅啟發培訓課程！這是一場塔羅深度培訓，專為您打造的個人化心靈探索之旅。在這四小時的課程中，您將與安老師深入探討塔羅牌的奧秘，學會如何解讀牌面的象徵意義，並掌握塔羅的應用技巧，啟發您的內在力量，指引您走向心靈成長之路。
  
  【服務費用】
  8,800元
  付款方式：在預約確認後，我們將提供您專屬的付款帳號。請在收到付款帳號後的三天內完成付款程序，以確保您的訂單有效。逾期未完成付款者，將取消訂單。
  
  【預約限制】
  預約限制：每月名額有限，請盡早預約確保您的位置。
  人數限制：課程設計為一對一培訓，提供專屬的學習體驗。
  
  【課程提醒】 
  這是一場個人化的心靈啟發之旅。請確認您已了解上課地點的詳細地址和相關交通資訊，提前安排好您的交通方式，以確保準時抵達上課地點。 
      `,
    },
  ],
} as const;

export const SERVICES = Object.values(SERVICE).flat();

export const ORDER = {
  IN_PROGRESS: [
    {
      service: {
        id: SERVICE.IN_PROGRESS[0].serviceId,
        name: SERVICE.IN_PROGRESS[0].name,
        serviceId: SERVICE.IN_PROGRESS[0].serviceId,
        duration: SERVICE.IN_PROGRESS[0].duration,
        currentAttendee: 1,
        attendee: SERVICE.IN_PROGRESS[0].attendee,
        location: SERVICE.IN_PROGRESS[0].location,
        supplier: SERVICE.IN_PROGRESS[0].supplier,
      },
      userId: 1,
      name: '花花',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2024-01-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2024-01-30T13:00',
      updateAt: '2024-01-01T00:00:00',
      id: 1,
    },
    {
      service: {
        id: SERVICE.IN_PROGRESS[0].serviceId,
        name: SERVICE.IN_PROGRESS[0].name,
        duration: SERVICE.IN_PROGRESS[0].duration,
        currentAttendee: 1,
        attendee: SERVICE.IN_PROGRESS[0].attendee,
        location: SERVICE.IN_PROGRESS[0].location,
        supplier: SERVICE.IN_PROGRESS[0].supplier,
      },
      userId: 2,
      name: '泡泡',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2024-01-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2024-01-30T14:20',
      updateAt: '2024-01-01T00:00:00',
      id: 2,
    },
    {
      service: {
        id: SERVICE.IN_PROGRESS[1].serviceId,
        name: SERVICE.IN_PROGRESS[1].name,
        duration: SERVICE.IN_PROGRESS[1].duration,
        currentAttendee: 1,
        attendee: SERVICE.IN_PROGRESS[1].attendee,
        location: SERVICE.IN_PROGRESS[1].location,
        supplier: SERVICE.IN_PROGRESS[1].supplier,
      },
      userId: 3,
      name: '毛毛',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2024-01-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2024-01-30T16:20',
      updateAt: '2024-01-01T00:00:00',
      id: 4,
    },
    {
      service: {
        id: SERVICE.IN_PROGRESS[2].serviceId,
        name: SERVICE.IN_PROGRESS[2].name,
        duration: SERVICE.IN_PROGRESS[2].duration,
        currentAttendee: 1,
        attendee: SERVICE.IN_PROGRESS[2].attendee,
        location: SERVICE.IN_PROGRESS[2].location,
        supplier: SERVICE.IN_PROGRESS[2].supplier,
      },
      userId: 4,
      name: '老皮',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2024-01-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2024-01-29T13:00',
      updateAt: '2024-01-01T00:00:00',
      id: 5,
    },
    {
      service: {
        id: SERVICE.IN_PROGRESS[2].serviceId,
        name: SERVICE.IN_PROGRESS[2].name,
        duration: SERVICE.IN_PROGRESS[2].duration,
        currentAttendee: 1,
        attendee: SERVICE.IN_PROGRESS[2].attendee,
        location: SERVICE.IN_PROGRESS[2].location,
        supplier: SERVICE.IN_PROGRESS[2].supplier,
      },
      userId: 5,
      name: '阿賢',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2024-01-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2024-01-05T13:00',
      updateAt: '2024-01-01T00:00:00',
      id: 6,
    },
    {
      service: {
        id: SERVICE.IN_PROGRESS[2].serviceId,
        name: SERVICE.IN_PROGRESS[2].name,
        duration: SERVICE.IN_PROGRESS[2].duration,
        currentAttendee: 1,
        attendee: SERVICE.IN_PROGRESS[2].attendee,
        location: SERVICE.IN_PROGRESS[2].location,
        supplier: SERVICE.IN_PROGRESS[2].supplier,
      },
      userId: 6,
      name: '荷馬',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2024-01-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2024-01-02T13:00',
      updateAt: '2024-01-01T00:00:00',
      id: 7,
    },
  ],
  END: [
    {
      service: {
        id: SERVICE.END[0].serviceId,
        name: SERVICE.END[0].name,
        duration: SERVICE.END[0].duration,
        currentAttendee: 1,
        attendee: SERVICE.END[0].attendee,
        location: SERVICE.END[0].location,
        supplier: SERVICE.END[0].supplier,
      },
      userId: 7,
      name: 'Doremi',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2023-12-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2023-12-30T13:00:00',
      updateAt: '2023-12-01T00:00:00',
      id: 8,
    },
    {
      service: {
        id: SERVICE.END[0].serviceId,
        name: SERVICE.END[0].name,
        duration: SERVICE.END[0].duration,
        currentAttendee: 1,
        attendee: SERVICE.END[0].attendee,
        location: SERVICE.END[0].location,
        supplier: SERVICE.END[0].supplier,
      },
      userId: 8,
      name: '小愛',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2023-12-01T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2023-12-30T14:00',
      updateAt: '2023-12-01T00:00:00',
      id: 9,
    },
    {
      service: {
        id: SERVICE.END[2].serviceId,
        name: SERVICE.END[2].name,
        duration: SERVICE.END[2].duration,
        currentAttendee: 1,
        attendee: SERVICE.END[2].attendee,
        location: SERVICE.END[2].location,
        supplier: SERVICE.END[2].supplier,
      },
      userId: 9,
      name: '羽月',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      note: {
        updateAt: '2023-12-10T13:50:00',
        content:
          '感謝您的預約，在到來之前可以先想好您要諮詢的內容，讓安老師能更精準的協助您！',
      },
      startAt: '2023-12-31T13:00',
      updateAt: '2023-12-01T00:00:00',
      id: 10,
    },
  ],
  REQUESTED: [
    {
      service: {
        id: SERVICE.IN_PROGRESS[0].serviceId,
        name: SERVICE.IN_PROGRESS[0].name,
        duration: SERVICE.IN_PROGRESS[0].duration,
        currentAttendee: SERVICE.IN_PROGRESS[0].currentAttendee,
        attendee: SERVICE.IN_PROGRESS[0].attendee,
        description: SERVICE.IN_PROGRESS[0].description,
        location: SERVICE.IN_PROGRESS[0].location,
        supplier: SERVICE.IN_PROGRESS[0].supplier,
        allday: SERVICE.IN_PROGRESS[0].allday,
        queue: SERVICE.IN_PROGRESS[0].queue,
      },
      userId: 8,
      name: '海綿寶寶',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      queues: [
        {
          currentAttendee: 1,
          startAt: '2024-01-30T14:30:00',
          disabled: true,
          reason: '此時段已有訂單',
        },
        {
          currentAttendee: 0,
          startAt: '2024-01-30T09:20:00',
          disabled: true,
          reason: '此時段為休假時間',
        },
        {
          currentAttendee: 0,
          startAt: '2024-01-29T09:20:00',
          disabled: false,
        },
      ],
      note: {
        updateAt: '',
        content: '',
      },
      startAt: '',
      updateAt: '2023-12-24T10:00:00',
      id: 11,
    },
    {
      service: {
        id: SERVICE.IN_PROGRESS[1].serviceId,
        name: SERVICE.IN_PROGRESS[1].name,
        duration: SERVICE.IN_PROGRESS[1].duration,
        currentAttendee: SERVICE.IN_PROGRESS[1].currentAttendee,
        attendee: SERVICE.IN_PROGRESS[1].attendee,
        address: SERVICE.IN_PROGRESS[1].location.address,
        description: SERVICE.IN_PROGRESS[1].description,
        location: SERVICE.IN_PROGRESS[1].location,
        supplier: SERVICE.IN_PROGRESS[1].supplier,
        allday: SERVICE.IN_PROGRESS[1].allday,
        queue: SERVICE.IN_PROGRESS[1].queue,
      },
      userId: 9,
      name: '派大星',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      queues: [
        {
          currentAttendee: 0,
          startAt: '2024-01-29T09:00:00',
          disabled: false,
        },
        {
          currentAttendee: 1,
          startAt: '2024-01-30T16:20:00',
          disabled: true,
          reason: '此時段已有訂單',
        },
        {
          currentAttendee: 0,
          startAt: '2024-01-21T10:00:00',
          disabled: false,
        },
      ],
      note: {
        updateAt: '',
        content: '',
      },
      startAt: '',
      updateAt: '2023-12-30T19:00:00',
      id: 12,
    },
    {
      service: {
        id: SERVICE.IN_PROGRESS[2].serviceId,
        name: SERVICE.IN_PROGRESS[2].name,
        duration: SERVICE.IN_PROGRESS[2].duration,
        currentAttendee: SERVICE.IN_PROGRESS[2].currentAttendee,
        attendee: SERVICE.IN_PROGRESS[2].attendee,
        description: SERVICE.IN_PROGRESS[2].description,
        location: SERVICE.IN_PROGRESS[2].location,
        supplier: SERVICE.IN_PROGRESS[2].supplier,
        allday: SERVICE.IN_PROGRESS[2].allday,
        queue: SERVICE.IN_PROGRESS[2].queue,
      },
      userId: 10,
      name: '章魚哥',
      SNSId: 'bukku_user_a',
      email: 'example@example.com',
      phone: '0900-000-000',
      comment: '',
      queues: [
        {
          currentAttendee: 0,
          startAt: '2024-01-26T13:00:00',
          disabled: false,
        },
        {
          currentAttendee: 1,
          startAt: '2024-01-27T13:00:00',
          disabled: false,
        },
        {
          currentAttendee: 0,
          startAt: '2024-01-28T13:00:00',
          disabled: false,
        },
      ],
      note: {
        updateAt: '',
        content: '',
      },
      startAt: '',
      updateAt: '2024-01-01T00:00:00',
      id: 13,
    },
  ],
} as const;

export const ORDERS = Object.values(ORDER).flat();
