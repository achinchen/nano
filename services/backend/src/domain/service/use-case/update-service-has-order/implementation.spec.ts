...
type Payload = Parameters<UpdateServiceHasOrderUseCase['execute']>[0];
...
import { ServiceHistory } from '../../entity';
import { UpdateServiceHasOrderUseCase } from './implementation';

const updateServiceHasOrderUseCase = new UpdateServiceHasOrderUseCase(
  mockServiceRepository
);

describe('UpdateServiceHasOrderUseCase', () => {
  const id = 12;
  const providerId = 1;
  const version = '1.0.0';
  const targetVersion = versionValueObject.bumpPatch(version);
  ...
  it('should return an error if the provider ID is missing', async () => {
    ...
    const result = await updateServiceHasOrderUseCase.execute(
      payload as Payload
    );
  ...
  it('should return Result object with an error if there is an error creating the service', async () => {
    ...
    const [error] = await updateServiceHasOrderUseCase.execute({
      id,
      providerId,
      ...updatedPayload,
    });
