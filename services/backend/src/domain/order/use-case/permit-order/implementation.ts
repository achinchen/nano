...
import { PermitOrderDTO } from '~backend/domain/order/dto';
import { INVALID, NOT_ALLOW } from '~backend/domain/order/error';

type PermitOrderResult = boolean;
export type Payload = PermitOrderDTO;
type Return = IResult<PermitOrderResult> | AppErrorUnexpected;

export class PermitOrderUseCase implements UseCase<Payload, Promise<Return>> {
  ...
  async execute(payload?: Payload): Promise<Return> {
    ...
    try {
      ...
      return Result.ok<PermitOrderResult>(true);
    } catch (error) {
