...
type Payload = Parameters<UpdateOrderNoteUseCase['execute']>[0];
...
const mockOrderAggregateRoot = {
  updateOrderNote: OrderAggregateRoot.updateOrderNote,
} as unknown as jest.Mocked<typeof OrderAggregateRoot>;

import { UpdateOrderNoteUseCase } from './implementation';

const updateOrderNoteUseCase = new UpdateOrderNoteUseCase(mockOrderRepository);

describe('UpdateOrderNoteUseCase', () => {
  ...
  test.each([
    ...
    async (payload, expected) => {
      const result = await updateOrderNoteUseCase.execute(payload as Payload);
  ...
  it('should return an error if the order does not exist', async () => {
    ...
    const result = await updateOrderNoteUseCase.execute(payload);
  ...
  it('should return a Result object with an error if there is an error updating the order note', async () => {
    mockOrderRepository.getById.mockResolvedValueOnce(mockOrder);

    const errorMessage = 'Error updating order note';
    mockOrderAggregateRoot.updateOrderNote.mockReturnValueOnce(
      Result.fail(errorMessage)
    );
    const [error] = await updateOrderNoteUseCase.execute(payload);
    expect(error).toBe(errorMessage);
  });

  it('should return a Result object with true if the order note is updated successfully', async () => {
    mockOrderRepository.getById.mockResolvedValueOnce(mockOrder);
    mockOrderAggregateRoot.updateOrderNote.mockReturnValueOnce(
    ...
    const [error, result] = await updateOrderNoteUseCase.execute(payload);
