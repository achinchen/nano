...
import { RejectOrderDTO } from '~backend/domain/order/dto';
import { INVALID, NOT_ALLOW } from '~backend/domain/order/error';

type RejectOrderResult = boolean;
export type Payload = RejectOrderDTO;
type Return = IResult<RejectOrderResult> | AppErrorUnexpected;

export class RejectOrderUseCase implements UseCase<Payload, Promise<Return>> {
  ...
  async execute(payload?: Payload): Promise<Return> {
    ...
    try {
      ...
      return Result.ok<RejectOrderResult>(true);
    } catch (error) {
